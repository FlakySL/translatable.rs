    ...
    fn parse(input: ParseStream) -> SynResult<Self> {
        ...
        let parsed_langauge_arg = match parse2::<LitStr>(language_arg.clone()) {
            Ok(literal) => match literal.value().parse::<Language>() {
                ...
                Err(_) => Err(SynError::new_spanned(
                    literal,
                    "This literal is an invalid ISO 639-1 string and cannot be parsed."
                ))?
            },
                ...
                let language_arg = input.parse::<Path>()?
                    ...
                    .map(|segment| match segment.arguments {
                        ...
                        other => Err(SynError::new_spanned(
                            other,
                            "A translation doesn't have generic arguments"
                        )),
                    })
